...
-- TODO: This is hacky
                _ ->
                    Err <| Json.Decode.Failure ("Can't decode " ++ name ++ ": object has too many keys") (Json.Encode.string name)
        )
...
decodeSumTwoElemArray name mapping =
    customDecoder (tuple2 Tuple.pair Json.Decode.string Json.Decode.value) (\( key, value ) -> decodeSumFinal name key value mapping)
...
decodeMap decKey decVal =
    let
        -- TODO: FIx
        decodeKeys =
            Result.mapError (\err -> Json.Decode.Failure err (Json.Encode.string "idontknow")) (resmapM decodeKey)
    ...
    let
        encKey_ x =
            case Json.Decode.decodeValue Json.Decode.string (encKey x) of
                -- TODO: Fix this
                Err e -> Json.Decode.errorToString e
                    --Json.Encode.encode 0 x
...
bind decoder fun = Json.Decode.andThen fun decoder
